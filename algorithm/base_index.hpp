...
namespace efanna {
    ...
    public:
        ...
        void getResults(std::vector<int> *knn_vec, long long n_vertices) {
            ...
            for (i = nn_results.begin(); i != nn_results.end(); i++) {
                ...
                for (j = i->begin(); j != i->end(); j++) {
                    int id = *j;
                    knn_vec[icount].push_back(id);
                    //std::cout<<icount<<" "<<id<<std::endl;
                }
                icount++;
            }
        }
...
//std::cout<<dim<<std::endl;
        ...
        void
        setSearchParams(int epochs, int init_num, int extend_to, int search_trees, int search_lv, int search_method) {
        ...
        void nnExpansion_kgraph(size_t K, const DataType *qNow, std::vector<unsigned int> &pool,
            ...
            for (int i = 0; i < Iter; i++) {
                ...
                while (k < L) {
                    ...
                    if (newflag.test(knn[k].id)) {
                        ...
                        for (size_t nnk = 0;
                             nnk < params_.K && neighbor != knn_graph[knn[k].id].rend(); neighbor++, nnk++) {
                            if (tbflag.test(neighbor->row_id))continue;
                            tbflag.set(neighbor->row_id);
                            newflag.set(neighbor->row_id);
                            float dist = distance_->compare(qNow, features_.get_row(neighbor->row_id),
                                                            features_.get_cols());
        ...
        void nnExpansion(size_t K, const DataType *qNow, std::vector<unsigned int> &pool, std::vector<int> &res) {
            ...
            for (int seg = 0; seg < segIter; seg++) {
                ...
                for (int j = 0; j < Jter; j++) {
                    ...
                    if (!tbflag.test(nn)) {
                        newflag.set(nn);
                        Candidate<DataType> c(nn,
                                              distance_->compare(qNow, features_.get_row(nn), features_.get_cols()));
                        Candidates.insert(c);
                    }
                }
                ...
                while (iter++ < SP.search_epoches) {
                    ...
                    for (size_t j = 0; j < ids.size(); j++) {
                        Candidate<DataType> c(ids[j], distance_->compare(qNow, features_.get_row(ids[j]),
                                                                         features_.get_cols()));
                        Candidates.insert(c);
                        newflag.set(ids[j]);
...
#pragma omp parallel for
            for (size_t i = 0; i < nhoods.size(); i++) {
                if (nhoods[i].found) {
                    unsigned maxl =
                            nhoods[i].Range + params_.S < nhoods[i].L ? nhoods[i].Range + params_.S : nhoods[i].L;
                    unsigned c = 0;
                    unsigned l = 0;
...
#pragma omp parallel for
        ...
        void refineGraph() {
            ...
            while (iter++ < params_.build_epoches) {
                ...
                update(l);
                f = clock();
                std::cout << "iteration " << iter << " time: " << (f - s) * 1.0 / CLOCKS_PER_SEC << " seconds"
                          << std::endl;
            }
            //calculate_norm();
...
#pragma omp parallel for
        ...
        typedef std::set<Candidate < DataType>, std::greater<Candidate < DataType>> >
        CandidateHeap;
