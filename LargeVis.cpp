...
void LargeVis::construt_knn()
{
	...
	FIndex<float> index(dataset, new L2DistanceAVX<float>(),
						efanna::KDTreeUbIndexParams(true, knn_trees, mlevel, epochs, checkK, L, knn_k, build_trees, S));
	clock_t s, f;
	s = clock();//start time
	index.buildIndex();
	f = clock();//end time
...
void LargeVis::run(long long out_d, long long n_thre, long long n_samp, long long n_prop, real alph, long long n_tree, long long n_nega, long long n_neig, real gamm, real perp,
				    int knn_tre, int mle,  int epo, int knn_L, int che, int k, int knn_S, int build_tre)
{
	gsl_rng_env_setup();
